...
- **七类原子**：文本 / 形状 / 图标 / 图片 / GIF 动态图 / 进度 / 容器，可拖拽、改属性、组合嵌套。
- **多页 + 双模式**：编辑模式（摆放、调属性）与桌面模式（静态展示 + 点击交互 + 触发器自动化）一键切换。
- **公式引擎**：`$expr$` 包裹求值，14 个内置函数（df/tf/tu/si/bi/tc/if/fl/mu/mi/ce/bp/gv/wg）；支持运算符、颜色、逻辑；可绑定系统 / 媒体 / RSS 数据。
...
- **图层混合与模糊**：7 种 KLWP 混合模式（像素着色器）+ 图层模糊，纯代码实现。
...
- 快捷键（修饰键均为 `Ctrl+Alt`）：
  - 退出程序：`Q`
  - 切换编辑 / 桌面模式：`E`
  - 上一页 / 下一页：默认未绑定快捷键，请在源码 `LumenWindow.RegisterHotKeys` 中自行设置
- 其余功能（网格档 / 预设 / 新建页 / 显隐覆盖层）无默认热键，通过右键菜单或托盘菜单操作
- 配置档管理：`右键菜单` → 配置档…
- 界面语言：`右键菜单` → 设置 → 语言
