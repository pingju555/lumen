...
{
    /// <summary>
    /// 公式函数注册表。实现类：df tf tu si bi gv if tc fl mi mu ce bp wg。
    /// 已删除：ts(并入tf)/tz/tz/dp、uc/re(并入tc内参命令)、rng(并入mu rnd)、ai/an(动作系统另行处理)。
    ...
    public static class FunctionRegistry
    {
        ...
        static FunctionRegistry()
        {
            ...
            // ---------- 媒体 / 颜色 / 外部数据 ----------
            Register("mi", Mi);
            Register("mu", Mu);   // 数学派发器（含 rnd 随机数内参命令）
            Register("ce", Ce);   // 颜色运算
            Register("bp", Bp);   // 调色板
        ...
        private static Value Bi(Value[] a, EvalContext ctx)
        {
            ...
            if (t == "time")
                return Value.Of(ctx.Provider.BatteryTime());
            return Value.Of("");
        }
        ...
        private static Value Mu(Value[] a, EvalContext ctx)
        {
            ...
            switch (op)
            {
                ...
                case "rnd":
                    {
                        var rnd = new Random();
                        double hi = a.Length > 2 ? a[2].AsNum() : 1;
                        double lo = a.Length > 1 ? a[1].AsNum() : 0;
                        return Value.Of(lo + rnd.NextDouble() * (hi - lo));
                    }
