...
namespace Lumen.Formula
{
    ...
    public class SystemDataProvider : IDataProvider
    {
        ...
        public int BatteryTime()
        {
            var s = new NativeMethods.SYSTEM_POWER_STATUS();
            if (NativeMethods.GetSystemPowerStatus(ref s))
                return s.BatteryLifeTime; // -1 = unknown
            return -1;
        }
        ...
        public double Dpi()
        {
            // 从注册表读取系统 DPI 缩放系数（LogPixels = 96 为 100%）
            try
            {
                using var key = Registry.CurrentUser.OpenSubKey(@"Control Panel\Desktop\WindowMetrics", false);
                if (key?.GetValue("AppliedDPI") is int logPixels && logPixels > 0)
                    return logPixels / 96.0;
            }
            catch { }
            // 回退：通过 SystemParameters 工作区与分辨率估算缩放
            double sw = SystemParameters.PrimaryScreenWidth;
            double aw = SystemParameters.WorkArea.Width;
            // 常见分辨率：1920 @100% → 1536 工作区（任务栏），1920 @125% → 1536 工作区（缩放后实际 1920）
            // 但这种方式不精确，仅回退
            return 1.0;
        }
