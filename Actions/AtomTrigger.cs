...
namespace Lumen.Actions
{
    ...
    /// </summary>
    public enum TriggerFireMode { Once = 0, While = 1 }

    /// <summary>
    /// 单个触发器：当布尔公式条件成立时自动执行一个「流程」（有序的一组动作，依次执行，无需点击）。
    /// 序列化为 "Condition\u0001Mode\u0001Action1\u0003Action2\u0003..."；触发器列表以 \u0002 连接
    /// （均使用不可键入的控制字符作分隔，避免与公式内容冲突）。
    /// 旧版单动作格式（无 \u0003）解析时自动归入流程首步，向后兼容。
    /// </summary>
    public class AtomTrigger
    {
        public string Condition = "";
        public List<AtomAction> Actions = new List<AtomAction>();
        public TriggerFireMode Mode = TriggerFireMode.Once;

        // 运行态（不持久化）：用于 Once 模式的上升沿检测
        private bool _wasTrue;

        /// <summary>
        /// 用给定上下文评估条件，返回本周期是否应触发流程。
        ...
        public static AtomTrigger Parse(string s)
        {
            if (string.IsNullOrWhiteSpace(s)) return new AtomTrigger();
            var parts = s.Split('\u0001');
            var t = new AtomTrigger();
            if (parts.Length > 0) t.Condition = parts[0] ?? "";
            if (parts.Length > 1 && int.TryParse(parts[1], out var m)) t.Mode = (TriggerFireMode)m;
            if (parts.Length > 2)
            {
                var joined = parts[2];
                if (joined.Contains('\u0003'))
                    ...
                    t.Actions = new List<AtomAction> { AtomAction.Parse(joined) }; // 旧版单动作：归入流程首步
            }
            return t;
        }

        public static string SerializeList(List<AtomTrigger> list)
        {
        ...
        public static List<AtomTrigger> ParseList(string s)
        {
            var list = new List<AtomTrigger>();
