...
### 修复
- **九宫格锚点 + 偏移 与拖拽关系正式定稿** — 确立四条规则：① 拖拽只改写 `OffsetX/Y`（反解烤回，锚点不变）；② 改锚点（属性面板魔方）时把**静态**偏移归零，新锚点 + 偏移 0 成为该锚点默认落点（`TopLeft+0`=屏幕左上、`Center+0`=屏幕正中、`BottomRight+0`=右下角），偏移体系重新相对新锚点计算；③ 删除 `RecalcPosition` 中 `TopLeft+偏移0` 短路分支，所有锚点统一走锚点数学（`TopLeft+任意偏移` 即「以屏幕左上角为原点的绝对坐标」），不再有「自由定位逃生舱」与常规路径不一致；④ 偏移被**公式/变量绑定**时禁用画布拖拽（保留绑定、清除绑定后方可拖动），避免拖拽结果下次重组被公式覆盖；公式/变量绑定的偏移在改锚点时**不清零**以保留其绑定
- **改锚点不重置绑定偏移** — `PropertyEditorPanel.BuildAnchorEditor` 锚点点击回调仅在偏移为**静态模式**时归零；公式/变量绑定的偏移原样保留，仅随新锚点基准重新解释落位（符合「保留绑定」语义）

---

## [v1.3.2] — 2026-07-20

> 修 v1.3.1 回归（内置手册不刷新）+ 配置默认位置改为便携（随 exe）。
...
- **修复反复切页时新页面变透明** — 根因在切页淡入：`OnPageChanged` 原先把**整个 `PageHost`**（含背景/网格层）的 `Opacity` 从 0 拉到 1 做淡入。背景层是 `PageHost` 的子元素，故淡入期间整页（含背景）被拉到透明；反复快速切页时页面长期停在接近 0 的透明度，表现为「新切到的页面成透明」。现改法：`PageHost.Opacity` 固定为 1（背景/网格层始终不透明），只对 `_canvasLayer.Root`（原子画布层）做 0→1 淡入，背景永不被淡化；`ComposeCurrentPage` 仍同步把当前页自身背景写入壁纸层，切页即时显示为正确底色
- **理顺九宫格锚点 + 偏移 与拖拽的关系** — 明确四条规则：① 拖拽只改写 `OffsetX/Y`（反解烤回，锚点不变）；② 改锚点（属性面板魔方）时把**静态**偏移归零，使新锚点 + 偏移 0 成为该锚点的默认落点（`TopLeft+0`=屏幕左上、`Center+0`=屏幕正中…），偏移体系重新相对新锚点计算；③ 删除 `RecalcPosition` 中 `TopLeft+偏移0` 的短路分支，所有锚点统一走锚点数学（`TopLeft` 即「以屏幕左上角为原点的绝对坐标」，不丢任何能力）；④ 偏移被**公式/变量绑定**时禁用拖拽（保留绑定，清除绑定后方可拖动），避免拖拽结果下次重组被公式覆盖。公式/变量绑定的偏移在改锚点时**不清零**以保留其绑定
