...
- **修复部件位置跳动（被清到 0,0）** — `Atom.RecalcPosition` 每次重组都把位置按 `anchor+offset` 重算并覆盖 `Bounds`。对采用默认 `TopLeft` 锚点且 `offset=0` 的原子（默认主页部件、未跑过拖拽回写的旧 profile 原子）会强制清到 `(0,0)`，表现为位置跳动/塌到左上角。现增加「绝对定位模式」判定：`TopLeft` 锚点且 `offsetX/offsetY` 均为 0 时，视为绝对坐标已存于 `Bounds`（拖拽/落点写入），不再按锚点重算；仅当锚点非 `TopLeft` 或存在非零偏移时才按锚点重定位（窗口尺寸变化等）
